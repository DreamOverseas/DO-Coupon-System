 ...
 * @params assigned_from -> who provides good/service with this coupon (Document ID / Name)
...
app.post('/create-active-coupon', async (req, res) => {
  const { title, description, expiry, uses_left, assigned_from, assigned_to, email, contact, type, user, hide, value } = req.body;
  ...
  try {
    const usingName = assigned_from.length <= 16; // As Name shall not exceed 16 digits, while document ID is typically 24
    var assigner = assigned_from;
